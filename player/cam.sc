...
(
   if(!query(player,'nbt','OnGround'),
      print(format('w [','d Camera','w ] ','y You must be on firm ground.'));
      exit();
   );
   if(query(player,'nbt','Air') < 300,
      print(format('w [','d Camera','w ] ','y You must be in air, not suffocating nor in liquids.'));
      exit();
   );
   if(player ~ 'is_burning',
      print(format('w [','d Camera','w ] ','y You must not be on fire.'));
      exit();
   );
   null
);
   ...
   , current_gamemode == 'survival' && !global_is_in_switching, // else if survival - switch to spectator
      ...
      if (global_survival_timeout > 0,
         ...
         for(range(global_survival_timeout, 0, -1), 
            schedule((global_survival_timeout+1-_)*20, _(outer(_)) -> print(format('w [','d Camera','w ]','y  On in','b  '+_,'y ...')))
         );
         player_name = p~'name';
...
(
   ...
   current_gamemode = p~'gamemode';
   if ( current_gamemode == 'spectator',
      if ( query(p,'effect','night_vision') != null,
         (
            modify(p, 'effect', 'night_vision', null);
            modify(p, 'effect', 'conduit_power', null);
            print(format('w [','d Camera','w ] ','y Night Vision off.'));
         ),
         modify(p, 'effect', 'night_vision', 999999, 0, false, false);
         modify(p, 'effect', 'conduit_power', 999999, 0, false, false);
         print(format('w [','d Camera','w ] ','y Night Vision on.'));
      ); 
   );
   exit();
);
...
(
   ...
   modify(player, 'gamemode', 'survival');
   print(format('w [','d Camera','w ] ','y Off.'));
);
...
(
   ...
   print(format('w [','d Camera','w ] ','y On.'));
);
...
(
   ...
   print(format('w [','d Camera','w ] ','y Cannot find a safe spot to land within 32 blocks.'));
   false;
);
